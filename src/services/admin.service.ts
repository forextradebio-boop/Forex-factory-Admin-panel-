...
export const adminService = {
  ...
  async getKycDocuments(): Promise<KycDocument[]> {
    ...
    return (res.data.kycRequests || []).map((kyc: any) => {
      ...
      return {
        ...
        userFullName: kyc.userId?.fullName || kyc.userId?.username || kyc.userId?.email || 'Unknown User',
  ...
  async getDeposits(): Promise<Deposit[]> {
    try {
      ...
      const mapped = (res.data.deposits || []).map((d: any) => ({
        id: d._id,
        userId: d.userId?._id || "",
        userFullName: d.userId?.fullName || d.userId?.username || d.userId?.email || "Unknown User",
  ...
  async getWithdrawals(): Promise<Withdrawal[]> {
    ...
    return (res.data.withdrawals || []).map((w: any) => ({
      ...
      userFullName: w.userId?.fullName || w.userId?.username || w.userId?.email || 'Unknown User',
